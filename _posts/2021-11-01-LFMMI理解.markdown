...
语音识别的目标是基于输入的观察值$O$(音频帧提取的特征值)，得到最匹配的词序列$W$，即概率使$P(W\mid O)$最大。根据贝叶斯公式，结合$P(O)$与$P(W)$互相独立的假设，最大化$P(W\mid O)$等价于最大化$P(O\mid W)P(W)$，其中$P(O\mid W)$为声学模型，$P(W)$为语言模型。
...
3. [[论文\]Sequence-discriminative training of DNNs笔记 | Chao Yang (placebokkk.github.io)](https://placebokkk.github.io/asr/2019/12/24/asr-paper-se-mmi-note.html)
